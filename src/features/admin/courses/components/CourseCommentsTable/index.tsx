...
// ─── Types ────────────────────────────────────────────────────────────────────
export type CourseCommentStatus = "pending" | "approved" | "rejected";

export interface AdminCourseCommentRow {
    id: string;
    author: string;
    content: string;
    created_at: string;
...
interface CourseCommentsTableProps {
    data: AdminCourseCommentRow[];
    onDelete: (comment: AdminCourseCommentRow) => void;
    onReply: (comment: AdminCourseCommentRow, reply: string) => void;
    onStatusChange: (id: string, status: CourseCommentStatus) => void;
}
...
interface ReplyModalProps {
    comment: AdminCourseCommentRow | null;
    open: boolean;
    onClose: () => void;
...
export function CourseCommentsTable({
    ...
    onReply,
    onStatusChange,
}: CourseCommentsTableProps) {
    const [replyTarget, setReplyTarget] = useState<AdminCourseCommentRow | null>(null);
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="text-right min-w-[160px]">عنوان دوره</TableHead>
                            <TableHead className="text-right min-w-[90px]">لینک دوره</TableHead>
                            ...
                            return (
                                <TableRow key={comment.id}>
                                    ...
                                    <TableCell className="text-sm max-w-[180px]">
                                        <span className="line-clamp-1">{comment.course_title}</span>
                                    </TableCell>

                                    {/* course link */}
                                    <TableCell>
                                        <a
                                            href={`/courses/${comment.course_slug}`}
                                            target="_blank"
